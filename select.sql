/* Consulta de funcionários: Escreva uma query para listar todos os funcionários ativos, mostrando as colunas id, nome, salario.
...
*/
SELECT c.id_cliente AS id, c.razao_social AS razao_social, COALESCE(SUM(p.valor_total), 0) AS total
FROM clientes c LEFT JOIN pedido p ON c.id_cliente = p.id_cliente
GROUP BY c.id_cliente
ORDER BY total DESC;

/* Situação por pedido: Escreva uma query que retorne a situação atual de cada pedido da base.
...
*/
SELECT ip.id_produto, SUM(ip.quantidade) AS quantidade_vendida, SUM(ip.preco_praticado * ip.quantidade) AS total_vendido,
COUNT(ip.id_pedido) AS pedidos, COUNT(DISTINCT p.id_cliente) AS clientes
FROM itens_pedido ip INNER JOIN pedido p ON ip.id_pedido = p.id_pedido
